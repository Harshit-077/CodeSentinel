...
export function ReportViewer({ report }: Props) {
  ...
  return (
    <div className="card animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-6 pb-5 border-b border-gray-800">
        ...
        <div className="flex gap-8">
          <ScoreRing
            ...
            label="Severity"
            color={
              (report.severity_score ?? 0) > 70
                ? "text-red-400"
                : (report.severity_score ?? 0) > 40
                  ? "text-yellow-400"
                  : "text-green-400"
            }
          />
      ...
      <div className="flex gap-1 mb-5 overflow-x-auto">
        {TABS.map(({ key, label, icon: Icon }) => (
          ...
          >
            ...
            {key === "bugs" && (
              <span
                className={`ml-1 text-[11px] px-1.5 rounded-full ${tab === key ? "bg-white/20 text-white" : "bg-orange-500/20 text-orange-400"}`}
              >
            ...
            {key === "security" && (
              <span
                className={`ml-1 text-[11px] px-1.5 rounded-full ${tab === key ? "bg-white/20 text-white" : "bg-red-500/20 text-red-400"}`}
              >
        ...
        <div className="space-y-5 animate-fade-in">
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-gray-800/50 rounded-xl p-4">
              ...
              {fr.risk_assessment &&
                Object.entries(fr.risk_assessment).map(([k, v]) => (
                  <div
                    key={k}
                    className="flex justify-between text-sm py-1 border-b border-gray-700/50 last:border-0"
                  >
          ...
          <div className="bg-gray-800/50 rounded-xl p-4">
            ...
            <div className="space-y-2">
              {(fr.key_findings ?? []).map(
                (
                  f: { category: string; finding: string; severity: string },
                  i: number,
                ) => (
        ...
        <div className="space-y-3 animate-fade-in">
          <div className="flex gap-4 text-sm text-gray-500 mb-2">
            <span>
              ...
              <strong className="text-gray-300">
                {report.bugs?.overall_code_quality}
              </strong>
            </span>
            <span>
              Anti-patterns:{" "}
              <strong className="text-gray-300">
                {report.bugs?.anti_patterns_detected?.length ?? 0}
              </strong>
            </span>
          </div>
          ...
          {(report.bugs?.issues ?? []).map((bug) => (
            ...
            >
              ...
              {bug.code_snippet && (
                <pre
                  className="text-xs bg-gray-900 rounded-lg p-3 overflow-x-auto
              ...
              <div
                className="text-xs text-green-400 bg-green-500/10 rounded-lg p-2.5
        ...
        <div className="space-y-3 animate-fade-in">
          <div className="flex gap-4 text-sm mb-2">
            {[
              {
                label: "Critical",
                count: report.security_issues?.critical_count,
                color: "text-red-400",
              },
              {
                label: "High",
                count: report.security_issues?.high_count,
                color: "text-orange-400",
              },
              {
                label: "Medium",
                count: report.security_issues?.medium_count,
                color: "text-yellow-400",
              },
              {
                label: "Low",
                count: report.security_issues?.low_count,
                color: "text-blue-400",
              },
      ...
      {tab === "docs" && (
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="flex-1">
              ...
              <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${report.docs_suggestions?.readme_score ?? 0}%`,
                  }}
                />
              </div>
            </div>
          ...
          <div className="bg-gray-800/50 rounded-xl p-4">
            ...
            <ul className="space-y-1.5">
              {(report.docs_suggestions?.quick_wins ?? []).map(
                (w: string, i: number) => (
                  <li
                    key={i}
                    className="flex items-start gap-2 text-sm text-gray-400"
      ...
      {tab === "actions" && (
        <div className="space-y-3 animate-fade-in">
          <p className="text-sm text-gray-500 mb-3">
            Sorted by priority — tackle these first
          </p>
          {(fr.action_items ?? [])
            .sort(
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3 mt-1.5 text-xs text-gray-500">
                      ...
                      <span>
                        Effort:{" "}
                        <strong className="text-gray-400">{item.effort}</strong>
                      </span>
                      <span>
                        Impact:{" "}
                        <strong className="text-gray-400">{item.impact}</strong>
                      </span>
                    </div>
                  </div>
                </div>
      ...
      {/* ── Evaluation Tab ── */}
      {tab === "evaluation" && <EvaluationPanel jobId={report.job_id} />}
    </div>
  );
}
